...
# ── T-Q 曲线与夹点分析 ──
from core.postprocess import build_heat_tq_curves, analyse_pinch
fn = lambda fluid, T, P: layer.properties.state("TP", T, P)["H"]
tq = build_heat_tq_curves(report, fn)              # 从报告构建吸热/放热 T-Q 曲线
result = analyse_pinch(                            # 夹点分析：输入 ProcessRecord 列表
    [r for _,r in report.by_edge if r.category == "heat_absorption"],
    [r for _,r in report.by_edge if r.category == "heat_rejection"],
    delta_T_min=10.0, enthalpy_fn=fn,
)
...
| [`core/cycle_performance.py`](core/cycle_performance.py) | 性能统计：过程归类、循环汇总（无外部依赖） |
| [`core/postprocess.py`](core/postprocess.py) | 二次处理：T-Q 曲线构建、夹点分析（`analyse_pinch`） |
...
| 性能统计（3×2 对比） | `pytest tests/test_cycle_performance.py -q` | `tests/ideal_vs_non_ideal_cycle_performance.png` |
| 夹点分析（2×2 对比） | `pytest tests/test_pinch_analysis.py -q` | `tests/pinch_analysis_ideal_vs_non_ideal.png` |

全量：`pytest tests/ -q`（共 8 个用例）。也可直接运行：
