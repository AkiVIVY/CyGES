...
report = layer.performance_report()            # 精简边过程归类、循环汇总与 T-Q 曲线（非理想已偏置时用非理想状态）

# ── 夹点分析 ──
from core.postprocess import compute_pinch
rej = next(c for c in report.heat_tq_curves if c.category == "heat_rejection")
abs = next(c for c in report.heat_tq_curves if c.category == "heat_absorption")
pinch = compute_pinch(rej, abs, delta_T_min=10.0)   # 平移吸热曲线，求夹点与重叠/非重叠区段
```
...
| [`core/cycle_performance.py`](core/cycle_performance.py) | 性能统计：过程归类、T-Q 曲线、循环汇总 |
| [`core/postprocess.py`](core/postprocess.py) | 二次处理：夹点分析（`compute_pinch`） |
