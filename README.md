...
non_ideal = layer.ensure_non_ideal()       # 快照：simplified + 有向组 + 组内深度
# 节点偏置（旧实现，待重写）：
from core.non_ideal_node_offsets_legacy import (
    apply_heat_pressure_offsets,
    apply_mechanical_isentropic_offsets,
)
apply_heat_pressure_offsets(non_ideal)     # σ：P ← P × σ^layer + 全表 PS 闭合
apply_mechanical_isentropic_offsets(non_ideal)  # η_is：PS → η → HP（须在换热之后）
```
...
| [`core/non_ideal_closed_cycle_layer.py`](core/non_ideal_closed_cycle_layer.py) | 非理想层：`NonIdealClosedCycleLayer`、有向组、组内深度 |
| [`core/non_ideal_node_offsets_legacy.py`](core/non_ideal_node_offsets_legacy.py) | **旧**节点偏置（`σ` / `η_is`），待替换 |
